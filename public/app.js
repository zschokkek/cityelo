document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchRankings() {
        fetch(`${API_BASE_URL}/api/cities/rankings`)
            ...
            .catch(error => {
                ...
                rankingsTableBody.innerHTML = '<tr><td colspan="5">Error loading rankings. Please try again.</td></tr>';
            });
    }
    ...
    function submitCitySuggestion() {
        const cityName = cityNameInput.value.trim();
        
        if (!cityName) {
            suggestionMessage.textContent = 'Please enter a city name';
        ...
        .then(data => {
            if (data.success) {
                cityNameInput.value = '';
                suggestionMessage.textContent = data.message || 'Thank you for your suggestion!';
                ...
                suggestionMessage.textContent = data.message || data.error || 'Failed to submit suggestion';
                suggestionMessage.className = 'error';
            }
        })
        .catch(error => {
            console.error('Error submitting suggestion:', error);
            suggestionMessage.textContent = 'An error occurred. Please try again.';
            suggestionMessage.className = 'error';
        });
    }
